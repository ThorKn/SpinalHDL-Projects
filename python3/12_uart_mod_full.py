...
class Barret:
    ...
    def calc_barret_modulus_2(self, z, mask):
        ...
        print("TEMP_7 : " + str(temp_7.to_bytes(int(self.width+1), 'big', signed=True).hex().upper()))
...
for i in range(10000):
    ...
    b = os.urandom(int(WIDTH))

    a_int = (int.from_bytes(a, 'big')) % barret.n
    b_int = (int.from_bytes(b, 'big')) % barret.n

    a = a_int.to_bytes(int(WIDTH), 'big')
    b = b_int.to_bytes(int(WIDTH), 'big')
