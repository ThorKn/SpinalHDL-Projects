...
public:
    ...
    Wrapper_1(const char * name){
      time = 0;
      waveEnabled = true;
      signalAccess[0] = new CDataSignalAccess(&(top.io_clk_out));
      signalAccess[1] = new CDataSignalAccess(&(top.clk));
      signalAccess[2] = new CDataSignalAccess(&(top.reset));

      #ifdef TRACE
      Verilated::traceEverOn(true);
      top.trace(&tfp, 99);
      tfp.open((std::string("/home/moss/Documents/vdfs-in-hardware/implementations/ulx3s_projects/extra_clockdiv/spinalHDL/./simWorkspace/ClkDiv/") + name + ".vcd").c_str());
      #endif
    }
