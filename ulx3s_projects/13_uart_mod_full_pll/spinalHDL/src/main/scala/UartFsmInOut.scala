...
class UartFsmInOut(width: Int) extends Component{
    val io = new Bundle{
        ...
        val toFnct = master Flow(Bits(((4*width)+16) bits))
    ...
    val fsmIn = new StateMachine{
        val inCounter = Reg(UInt(math.ceil(log2(((4*width)+16)/8)).toInt bits)) init (0)
        val buffer = Reg(Bits((4*width)+16 bits)) init(0)
        io.toFnct.payload := 0
