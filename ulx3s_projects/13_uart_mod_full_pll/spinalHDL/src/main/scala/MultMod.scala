...
class MultMod(width: Int) extends Component{
    val io = new Bundle{
        val fromUart = slave Flow(Bits(((4*width)+16) bits))
    ...
    val fsm = new StateMachine{
        ...
        // val mask          = B(width*2 bits, default -> true) >> (width - (64))
        ...
        val stateA : State = new State with EntryPoint{
            whenIsActive {
                when(io.fromUart.valid) {
                    ...
                    b      := io.fromUart.payload.asBits((2*width)-1 downto width)
                    mod_n  := io.fromUart.payload.asBits((3*width)-1 downto (2*width))
                    mue    := io.fromUart.payload.asBits((4*width)+8-1 downto (3*width))
                    k      := io.fromUart.payload.asBits((4*width)+16-1 downto (4*width)+8)
                    goto(stateB)
                }
            }
        }
        ...
        val stateF : State = new StateDelay(cyclesCount=1){
            whenIsActive {
                // mask := B(width*2 bits, default -> true) >> (2*width - (16*(k.asUInt+1)))
                when(temp_7 < 0){
                    temp_8 := (temp_7 + mask.asSInt).asUInt
                    io.temp_corr_flag := 1
                }.otherwise{
        ...
        val stateG : State = new StateDelay(cyclesCount=1){
            whenIsActive {
        ...
        val stateH : State = new StateDelay(cyclesCount=2){
            whenIsActive {
                //io.toUart.payload := temp_8.asBits.resize(width)
