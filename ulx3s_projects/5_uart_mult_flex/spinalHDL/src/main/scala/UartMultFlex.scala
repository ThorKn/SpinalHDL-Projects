...
class UartMultFlex(width: Int) extends Component{
    val io = new Bundle{
        val uart = master(Uart())
    }

    val uartfsm = new UartFsmInOut(width)
    val mult = new Mult(width)

    uartfsm.io.uart <> io.uart
    uartfsm.io.toFnct >> mult.io.fromUart
    mult.io.toUart >> uartfsm.io.fromFnct
}
