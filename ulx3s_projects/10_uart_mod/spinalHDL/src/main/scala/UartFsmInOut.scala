...
class UartFsmInOut(width: Int) extends Component{
    val io = new Bundle{
        ...
        val toFnct = master Flow(Bits((width+width/2+16) bits))
    ...
    val fsmIn = new StateMachine{
        val inCounter = Reg(UInt(math.ceil(log2((width + width/2 + 16)/8)).toInt bits)) init (0)
        val buffer = Reg(Bits(width + width/2 + 16 bits)) init(0)
        io.toFnct.payload := 0
        ...
        val inIncCounter : State = new State{
            whenIsActive{
                ...
                when(inCounter === ((width + width/2 + 16)/8-1)){
                    goto(inBufToPayload)
                }.otherwise{
                    goto(inReadByte)
                }
